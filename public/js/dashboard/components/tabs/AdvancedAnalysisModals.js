...
// --- 1. Peer Comparison Modal (rich version with batch + watchlist) ---
const PeerComparisonModal = window.PeerComparisonModal || (({ symbols = [], onClose }) => {
    ...
    const getBestValue = (key, values) => {
        ...
        if (higherIsBetter.includes(key)) return Math.max(...validValues);
        if (lowerIsBetter.includes(key)) return Math.min(...validValues);
        return null;
    };
            ...
            <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl max-w-7xl w-full border border-gray-700 shadow-2xl my-8">
                <div className="p-6 border-b border-gray-700 flex justify-between items-center bg-gradient-to-r from-blue-900/30 to-purple-900/30">
                    <div>
                        ...
                        <p className="text-gray-400 text-sm mt-1">{symbols.join(' vs ')}</p>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {results.map(stock => (
                                <tr key={stock.symbol} className="hover:bg-gray-800/50 transition-colors">
                                    ...
                                    <td className="p-4 text-white">${typeof stock.price === 'number' ? stock.price.toFixed(2) : stock.price}</td>
                                    <td className="p-4">
        ...
        return (
            <BaseModal title={`Consensus Analystes: ${symbol}`} onClose={onClose}>
                <div className="text-gray-300">
                    Graphiques indisponibles (Recharts non chargé). Vérifiez le CDN pour afficher le donut.
                </div>
            </BaseModal>
        );
    }
    ...
    if (!hasBarCharts) {
        return (
            <BaseModal title={`Calendrier Earnings: ${symbol}`} onClose={onClose}>
                <div className="text-gray-300">
                    Graphiques indisponibles (Recharts non chargé). Les données seront affichées dès que le CDN est accessible.
                </div>
            </BaseModal>
        );
    }
