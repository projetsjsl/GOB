...
const GroupChatTab = ({ isDarkMode = true, dashboardTab, onDashboardTabChange }) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/groupchat/config')
      ...
      .catch((err) => {
        console.warn(' GroupChat: Erreur chargement config, utilisation du mode simulation:', err);
        setProviders({ 
          simulation: { 
            id:'simulation', 
            name:'Simulation', 
    ...
    fetch('/api/groupchat/admin')
      ...
      .catch((err) => {
        console.warn(' GroupChat: Erreur chargement themes:', err);
      });
    ...
    fetch('/api/groupchat/workflows')
      ...
      .catch((err) => {
        console.warn(' GroupChat: Erreur chargement workflows:', err);
      });
  }, []);
  ...
  const icons = { navigate:'', click:'', type:'', scroll:'', extract:'', wait:'', press:'', hover:'' };
  const fmt = (s) => s.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\n/g,'<br>');

  // Styles adaptes au theme du dashboard
  const themeStyles = {
