      ...
      <main style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        {activeTab === 'admin' && <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, padding:20, overflow:'auto' }}>
            ...
            {adminSection === 'test' && <div><h3 style={{ fontSize:18, fontWeight:700, marginBottom:16, color:themeStyles.text }}>🧪 Tests</h3><button onClick={()=>runTest('all')} disabled={testing} style={{ padding:'8px 16px', borderRadius:'8px', border:'none', background: testing ? themeStyles.surface : 'linear-gradient(135deg,#00e5ff 0%,#b388ff 50%,#ff80ab 100%)', color: testing ? 'var(--theme-text-secondary, #a0a0b0)' : '#000', cursor: testing ? 'not-allowed' : 'pointer', fontWeight:600, marginBottom:16 }}>{testing ? '🔄 Testing...' : '🧪 Test All'}</button>{testResults && <div style={{ padding:12, background:themeStyles.surface, borderRadius:'12px', border:`1px solid ${themeStyles.border}` }}>{testResults.tests?.map((t, i) => <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom: i < testResults.tests.length-1 ? `1px solid ${themeStyles.border}` : 'none' }}><span style={{ fontSize:13, color:themeStyles.text }}>{t.name}</span><span style={{ padding:'4px 8px', borderRadius:'4px', background: t.status==='pass' ? 'rgba(0,230,118,0.1)' : t.status==='fail' ? 'rgba(255,82,82,0.1)' : 'rgba(255,196,0,0.1)', color: t.status==='pass' ? '#00e676' : t.status==='fail' ? '#ff5252' : '#ffc400', fontSize:10, fontWeight:600 }}>{t.status==='pass' ? '✓' : t.status==='fail' ? '✗' : '○'} {t.message}</span></div>)}{testResults.summary && <div style={{ marginTop:12, fontSize:12, color:'var(--theme-text-secondary, #a0a0b0)' }}>{testResults.summary}</div>}</div>}</div>}
          </div>
        </div>}
      </main>
    </div>
