...
const AskEmmaTab = React.memo(({
                ...
                const [emmaMaxTokens, setEmmaMaxTokens] = useState(16000); // 🚀 Longueur augmentée pour analyses ULTRA-DÉTAILLÉES (par défaut 16k tokens)
                ...
                // ✅ Utiliser une Map pour gérer plusieurs intervalles de typing simultanés (crucial pour SMS multi-segments)
                const typingIntervalsRef = useRef(new Map());
                // Prompt chargé depuis /api/admin/emma-config (section=prompts, key=emmaSystemPrompt)
                ...
                const initializeEmma = async () => {
                    try {
                        ...
                        // Charger le prompt depuis emma-config API (centralisé)
                        ...
                        try {
                            ...
                            if (promptResponse.ok) {
                                ...
                                if (promptData?.config?.value) {
                                    ...
                                    setPromptSource('api');
                                    console.log('✅ Emma prompt chargé depuis API (emma-config)');
                                } else {
                                    ...
                                    if (savedPrompt) {
                                        ...
                                        console.log('⚠️ Emma prompt chargé depuis localStorage (API vide)');
                                    } else {
                                        console.log('ℹ️ Emma prompt: utilisation du défaut (API et localStorage vides)');
                                    }
                                }
                            } else {
                                ...
                                if (savedPrompt) {
                                    setEmmaPrompt(savedPrompt);
                                    setPromptSource('localStorage');
                                    console.log('⚠️ Emma prompt chargé depuis localStorage (API indisponible)');
                                }
                            }
                        } catch (promptError) {
                            console.warn('⚠️ Erreur chargement prompt depuis API:', promptError.message);
...
Prête à accompagner l'équipe dans leurs décisions d'investissement ?`;
                ...
                const startTypingEffect = (messageId, fullContent) => {
                    // Nettoyer l'intervalle spécifique à ce message s'il existe déjà
                    ...
                    let currentIndex = 0;
                    // Vitesse ultra-rapide pour donner une sensation de temps réel
                    const typingSpeed = 5; 
