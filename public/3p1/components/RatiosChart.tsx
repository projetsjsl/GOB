...
export const RatiosChart: React.FC<RatiosChartProps> = ({ data }) => {
  // Préparation des données : Calcul des moyennes annuelles pour P/E et P/CF
  const chartData = data
    .filter(row => row.priceHigh > 0 && row.priceLow > 0) // On s'assure qu'il y a des prix pour calculer les ratios
    .map(row => {
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {/* Composant Brush pour le Zoom et le Pan */}
            <Brush 
              dataKey="year" 
              height={30} 
              stroke="#cbd5e1" 
