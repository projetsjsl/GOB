...
export const DataQualityReport: React.FC<DataQualityReportProps> = ({ data, assumptions, ticker }) => {
  ...
  // Detecter les outliers avec details
  const outlierAnalysis = useMemo(() => {
    ...
    const outliers: OutlierInfo[] = validTargets.map(target => {
      ...
      if (deviation > threshold) {
        reason = `Ecart de ${formatPercent(deviationPercent)} par rapport a la mediane (${formatCurrency(median)})`;
      }
  ...
  // Analyser les donnees historiques pour detecter les valeurs aberrantes
        ...
        if (value > 0) {
          ...
          if (deviation > threshold) {
            outliers.push({
              ...
              average: mean,
              deviation,
              reason: `Ecart de ${formatPercent((deviation / mean) * 100)} par rapport a la moyenne`
            });
          }
        }
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-4 rounded-lg shadow-lg">
        <h2 className="text-xl font-bold flex items-center gap-2">
          ...
          Rapport de Qualite des Donnees - {ticker}
        </h2>
        <p className="text-blue-100 text-sm mt-1">
          Analyse complete des donnees aberrantes, exclusions et qualite des donnees historiques
        </p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
        ...
        <div className="mt-4 flex gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span>Metrique incluse</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-red-500 rounded"></div>
            <span>Metrique exclue (aberrante)</span>
          </div>
        </div>
      </div>

      {/* Details des Outliers */}
      {outlierAnalysis.outliers.filter(o => o.isExcluded).length > 0 && (
        <div className="bg-white p-4 rounded-lg shadow border border-red-200">
          <h3 className="text-lg font-bold text-red-800 mb-4 flex items-center gap-2">
            <ExclamationTriangleIcon className="w-5 h-5" />
            Metriques Aberrantes Detectees ({outlierAnalysis.outliers.filter(o => o.isExcluded).length})
          </h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-red-50">
                <tr>
                  ...
                  <th className="p-2 text-right">Mediane</th>
      ...
      {outlierAnalysis.outliers.filter(o => !o.isExcluded).length > 0 && (
        <div className="bg-white p-4 rounded-lg shadow border border-green-200">
          <h3 className="text-lg font-bold text-green-800 mb-4 flex items-center gap-2">
            ...
            Metriques Valides ({outlierAnalysis.outliers.filter(o => !o.isExcluded).length})
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {outlierAnalysis.outliers.filter(o => !o.isExcluded).map((outlier, idx) => (
              <div key={idx} className="bg-green-50 p-3 rounded border border-green-200">
                ...
                <div className="text-xs text-gray-500 mt-1">Ecart: {formatPercent(outlier.deviationPercent)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Graphique des Donnees Historiques */}
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
        <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
          <LineChart className="w-5 h-5 text-blue-600" />
          Evolution des Donnees Historiques
        </h3>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
      ...
      {historicalOutliers.length > 0 && (
        <div className="bg-white p-4 rounded-lg shadow border border-orange-200">
          <h3 className="text-lg font-bold text-orange-800 mb-4 flex items-center gap-2">
            ...
            Valeurs Aberrantes dans les Donnees Historiques ({historicalOutliers.length})
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div className="text-sm text-blue-600 font-semibold mb-1">Mediane des Prix Cibles</div>
          <div className="text-2xl font-bold text-blue-800">{formatCurrency(outlierAnalysis.median)}</div>
        </div>
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          <div className="text-sm text-purple-600 font-semibold mb-1">Ecart-Type</div>
          <div className="text-2xl font-bold text-purple-800">{formatCurrency(outlierAnalysis.stdDev)}</div>
        </div>
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          <div className="text-sm text-green-600 font-semibold mb-1">Metriques Valides</div>
