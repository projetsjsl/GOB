...
interface SiteConfig {
  url: string; name: string; icon: string; selector: string;
  color: { bg: string; accent: string; text: string };
  generate: (q: string) => any[];
}
...
function genSVG(site: SiteConfig, query: string, step: string, prog: number): string {
  const { bg, accent, text } = site.color;
  return `data:image/svg+xml;base64,${Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="800">
...
function detect(task: string): { site: SiteConfig; name: string; query: string } {
    ...
    async start(controller) {
      ...
      try {
        ...
        for (let i = 0; i < actions.length; i++) {
          ...
          send({ type: 'action_complete', index: i, success: true, screenshot: genSVG(site, query, steps[i], (i+1)/actions.length), url,
        ...
        send({ type: 'complete', results, finalUrl: url, message: `✅ ${results.length} results`, mode: 'simulation',
...
export async function GET() {
  return Response.json({ name: 'JSLAI Ultimate Simulation', version: '5.0', status: 'ready', cost: 'FREE',
