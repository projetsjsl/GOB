...
// Known 3P1 tables (initial set, will be updated dynamically)
let KNOWN_TABLES = [
...
export class SupabaseAgent extends BaseAgent {
    ...
    async initClient() {
        if (SUPABASE_URL && SUPABASE_KEY) {
            ...
            // Dynamically discover tables to stay updated
    ...
    /**
     * Dynamically discover tables in the public schema
     */
    async discoverTables() {
        try {
            // Use RPC or raw SQL if allowed, otherwise we stick to KNOWN_TABLES
            // Since we don't have a direct 'list tables' method in JS client, 
            // we try to query information_schema if we have service role key
            const { data, error } = await this.supabase
                .from('pg_tables') // This might not work depending on RLS/Permissions
                .select('tablename')
                .eq('schemaname', 'public');

            if (!error && data) {
                const discovered = data.map(t => t.tablename);
                // Merge discovered tables with known ones, avoiding duplicates
                KNOWN_TABLES = [...new Set([...KNOWN_TABLES, ...discovered])];
                console.log(`📡 [Supabase Agent] Discovered ${discovered.length} tables from DB`);
            }
        } catch (e) {
            console.log('ℹ️ [Supabase Agent] Table discovery via pg_tables limited, using default registry');
        }
    }
