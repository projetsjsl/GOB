...
function detectIntent(message, context = {}) {
  // Normaliser le message et retirer préfixes optionnels / ou #
  let normalized = message.trim();

  // ✨ Support des préfixes / ou # (comme /aide, #prix AAPL, /analyse MSFT)
  ...
  if (!normalized) {
    return {
      ...
      clarification: "Message vide. Envoyez 'Aide' ou '/aide' pour voir les commandes.",
    };
  }
