...
const INTENT_PATTERNS = {
  ...
  ANALYSE: {
    patterns: [
      // Note: longer alternatives first (courte before court, complète before complet)
      /^(analyse|analyser|analysis)\s+(?<modifier>courte|court|rapide|complète|complet|détaillée|détaillé)?\s*(?<ticker>[A-Z]{1,5})(?:\s|$)/i,
  ...
  DONNEES: {
    ...
    extractors: {
      ...
      type: (match) => match.groups?.type?.toLowerCase() || null,
