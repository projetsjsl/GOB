...
function buildDetailedHelp() {
  return `📊 EMMA SMS - 29 COMPÉTENCES

💡 PRÉFIXES (optionnels):
Vous pouvez utiliser / ou # devant toute commande
Ex: "/aide", "#prix AAPL", "/marchés"

🔍 ANALYSES ACTIONS:
• Prix AAPL - Prix temps réel
• Fondamentaux AAPL - P/E, ROE, marges
• RSI AAPL - Analyse technique
• News AAPL - Actualités récentes
• Analyse AAPL - Analyse complète détaillée
• AAPL vs MSFT - Comparaison
• Résultats AAPL - Earnings
• Recommandation AAPL - Avis analystes

📈 MARCHÉS:
• Marchés - Indices (S&P, Nasdaq, Dow)
• Secteur tech - Performance secteur
• Portefeuille - Votre watchlist

🌍 ÉCONOMIE:
• Inflation US - Données macro
• Politique Fed - Taux directeur

💼 STRATÉGIE:
• Stratégie investissement - Conseils
• Risque AAPL - Profil risque
• Gestion risque - Principes clés

💎 VALORISATION:
• Valorisation AAPL - Fair value
• Top croissance - Screening actions
• Méthodologie DCF - Valorisation
...
💱 AUTRES ACTIFS:
• USD/EUR - Forex
• Obligations US - Taux bonds

🌱 ESG:
• ESG AAPL - Score environnemental

Envoyez "LISTE" ou "/liste" pour exemples détaillés`;
}

function buildSkillsExamples() {
  return `📋 EXEMPLES DÉTAILLÉS (28 compétences)

📊 ACTIONS:
"Prix AAPL" → Prix temps réel + variation
"Fondamentaux AAPL" → P/E, ROE, marges, dette
"RSI AAPL" → Indicateurs techniques (RSI, MACD)
"News AAPL" → Actualités récentes
"Analyse AAPL" → Analyse complète (jusqu'à 1500 chars!)
"AAPL vs MSFT" → Comparaison détaillée
"Résultats AAPL" → Earnings + surprise
"Recommandation AAPL" → Consensus analystes

📈 MARCHÉS:
"Marchés" → S&P 500, Nasdaq, Dow Jones
"Secteur tech" → Performance Technology
"Portefeuille" → Votre watchlist

🌍 ÉCONOMIE & POLITIQUE:
"Inflation US" → CPI, tendance inflation
"Politique Fed" → Taux directeur, politique monétaire

💼 STRATÉGIE:
"Stratégie investissement" → Conseils long terme
"Risque AAPL" → Volatilité, profil risque
"Gestion risque" → Diversification, gestion

💎 VALORISATION:
"Valorisation AAPL" → Fair value, DCF
"Top croissance" → Screening actions croissance
"Méthodologie DCF" → Explication valorisation

💰 CALCULS:
"Calcul prêt 300k 25 ans 4.9%" → Paiements mensuels

💱 FOREX & BONDS:
"USD/EUR" → Taux de change actuel
"Obligations US" → Rendement T-bonds

🌱 ESG:
"ESG AAPL" → Score environnemental

✨ NOUVEAUTÉ: Analyses jusqu'à 1500 caractères!
