...
3. Max 280 caractères
4. Français clair, phrases courtes
...
`;
  ...
  switch (intent) {
    ...
    case 'COMPREHENSIVE_ANALYSIS':
      ...
      data = `${p.companyName || sourceData.ticker}: Prix ${q.price}$ (${q.changesPercentage}%), P/E ${r.peRatio}, ROE ${r.returnOnEquity}%, Secteur ${p.sector}`;
      task = 'Analyse complète en 3 phrases max (prix, valorisation, avis).';
      break;
...
function postProcessSMS(formatted, sourceData) {
  ...
  // Tronquer si besoin
  const maxTextLength = SMS_LIMITS.maxSMS - source.length - 10;
  ...
  if (finalText.length > SMS_LIMITS.maxSMS) {
    ...
    return {
      text: `${text}...\n\nSource: ${source}`,
      truncated: true,
      metadata: { originalLength: formatted.text.length, finalLength: finalText.length },
    };
  }
