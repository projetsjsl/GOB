...
/**
 * Recherche aperçu marchés via Perplexity
 */
async function searchMarketOverview() {
  const query = 'Situation actuelle des marchés boursiers (S&P 500, Nasdaq, Dow). Prix et variations en 2-3 phrases max.';
  return searchPerplexity(query, { maxTokens: 350 });
}

/**
 * Recherche analyse secteur
 */
async function searchSectorAnalysis(sector) {
  const query = `Performance actuelle du secteur ${sector}. Tendance + leaders en 2 phrases max.`;
  return searchPerplexity(query, { maxTokens: 300 });
}

/**
 * Recherche impact politique
 */
async function searchPoliticalImpact(topic) {
  const query = `Impact de ${topic} sur les marchés financiers. Résumé en 2 phrases max.`;
  return searchPerplexity(query, { maxTokens: 250 });
}

// Mettre à jour exports
module.exports.searchMarketOverview = searchMarketOverview;
module.exports.searchSectorAnalysis = searchSectorAnalysis;
module.exports.searchPoliticalImpact = searchPoliticalImpact;
