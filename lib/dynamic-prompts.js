 ...
 * 
 * ARCHITECTURE: Uses configManager to load prompts from Supabase (emma_system_config table)
 * Falls back to hardcoded file if DB is empty.
 */

import { CFA_SYSTEM_PROMPT } from '../config/emma-cfa-prompt.js'; // Fallback only
...
export class DynamicPromptsSystem {
    constructor() {
        ...
        // Fallback: File-based prompt (used if Supabase is empty)
        this.basePromptLegacyFile = CFA_SYSTEM_PROMPT;
        
        // Cached prompt from Supabase (loaded async)
        this.basePromptLegacy = null;
        this._loadBasePromptFromConfig();
...
`
    ...
    /**
     * Load base prompt from Supabase via configManager
     * Falls back to file-based prompt if DB is empty or unavailable
     */
    async _loadBasePromptFromConfig() {
        try {
            // Try to get CFA prompt from Supabase
            const cfaPrompt = await configManager.get('prompts', 'cfa_identity', null);
            
            if (cfaPrompt) {
                this.basePromptLegacy = cfaPrompt;
                console.log('✅ [DynamicPrompts] Base prompt loaded from Supabase (emma_system_config)');
            } else {
                // Fallback to file
                this.basePromptLegacy = this.basePromptLegacyFile;
                console.log('ℹ️ [DynamicPrompts] Using file-based fallback prompt (DB empty)');
            }
        } catch (error) {
            // Fallback to file on error
            this.basePromptLegacy = this.basePromptLegacyFile;
            console.warn('⚠️ [DynamicPrompts] ConfigManager error, using file fallback:', error.message);
        }
    }

    /**
     * Get the base prompt (with lazy loading from config)
     * @returns {Promise<string>} Base prompt from Supabase or file fallback
     */
    async getBasePrompt() {
        // If not loaded yet, load now
        if (this.basePromptLegacy === null) {
            await this._loadBasePromptFromConfig();
        }
        return this.basePromptLegacy || this.basePromptLegacyFile;
    }
