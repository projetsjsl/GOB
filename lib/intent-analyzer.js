...
export class HybridIntentAnalyzer {
    ...
    _analyzeLocal(userMessage, context) {
        ...
        // 1. Extraire les tickers du message actuel
        let tickers = this._extractTickers(userMessage);

        // ✅ FIX BUG 4: Si aucun ticker dans message actuel, chercher dans l'historique récent
        if (tickers.length === 0 && context.conversationHistory && context.conversationHistory.length > 0) {
