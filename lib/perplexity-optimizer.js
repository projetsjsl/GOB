...
RÉPONSE:`;
    ...
    _countMetrics(responseText) {
        ...
        // Compter chiffres avec unités (formats: 123.45, 1,234.56, 1 234,56)
        metrics.numbers = (responseText.match(/\d{1,3}([,\s]\d{3})*([.,]\d+)?/g) || []).length;

        // Compter pourcentages
        metrics.percentages = (responseText.match(/\d+([.,]\d+)?\s*%/g) || []).length;

        // Compter montants $ (formats: $123.45, 123,45$)
        metrics.dollars = (responseText.match(/(\$\s*\d+([.,]\d+)?|\d+([.,]\d+)?\s*\$)/g) || []).length;

        // Compter ratios (format "32,5x" ou "2.5x")
        metrics.ratios = (responseText.match(/\d+([.,]\d+)?x/gi) || []).length;

        // Compter comparaisons ("vs", "versus", "par rapport")
        metrics.comparisons = (responseText.match(/\b(vs|versus|par rapport à|comparé à|contre)\b/gi) || []).length;
    ...
    /**
     * Note le nombre de métriques (A+ à F)
     */
    ...
    _validateStructure(responseText, outputMode) {
        if (outputMode !== 'comprehensive_analysis') {
            return { passed: true, sections: [], coverage: 100 };
        }
        ...
        for (const section of requiredSections) {
            // Recherche flexible (emoji ou texte, avec ou sans ##)
            const regex = new RegExp(`(#{1,3}|\\*\\*)?\\s*(📊|💰|💼|📈|🎯|📰|⚖️)?\\s*${section}`, 'i');
     ...
     * AMÉLIORÉ: Validation multi-dimensions avec score global
     */
    _validateResponse(response, outputMode, intentData) {
        // 1. Validation métriques obligatoires (existante améliorée)
        const requiredMetrics = this._getRequiredMetricsList(outputMode, intentData);
        const missing = [];
        ...
        const scores = {
            ...
            metricsQuantity: metricsCount.passed ? 100 : Math.min((metricsCount.total / 20) * 100, 100),
        ...
        return {
            ...
            grade: this._getGlobalGrade(globalScore),
            ...
            metricsPresence: {
                ...
                coverage: Math.round(scores.metricsPresence)
            },
            ...
            recommendations: this._getImprovementRecommendations(globalScore, scores)
        };
    }
     ...
     * Grade global (A+ à F)
     */
    _getGlobalGrade(score) {
        if (score >= 95) return 'A+ (Excellent)';
        if (score >= 90) return 'A (Très bon)';
        if (score >= 85) return 'A- (Bon)';
        if (score >= 80) return 'B+ (Satisfaisant)';
        if (score >= 75) return 'B (Passable)';
        if (score >= 70) return 'C+ (Limite)';
        if (score >= 60) return 'C (Insuffisant)';
        return 'F (Échec)';
    }
    ...
    _getImprovementRecommendations(globalScore, scores) {
        if (globalScore >= 90) return ['✅ Excellent travail ! Qualité exceptionnelle.'];

        const recs = [];

        if (scores.metricsPresence < 80) {
            recs.push('❌ Métriques manquantes - vérifier toutes les données fournies');
        }

        if (scores.metricsQuantity < 80) {
            recs.push('📊 Ajouter plus de chiffres/ratios (minimum 20 requis)');
        }

        if (scores.length < 80) {
            recs.push('📏 Réponse trop courte - approfondir l\'analyse (1500+ mots)');
        }

        if (scores.structure < 80) {
            recs.push('📋 Sections manquantes - suivre structure obligatoire (7 sections)');
        }

        if (recs.length === 0) {
            recs.push('⚠️ Score correct mais améliorable - enrichir l\'analyse');
        }
