...
export function adaptForSMS(text, context = {}) {
  ...
  // 7. AJOUTER SOURCES D'ACTUALITÉS de manière amicale (1-2 liens max)
  // Extraire les citations de Perplexity pour partage amical
  const citations = context.citations || [];

  if (citations.length > 0) {
    // Filtrer les sources fiables (Bloomberg, Reuters, WSJ, etc.)
    const reliableSources = citations.filter(citation =>
      ...
      (
        ...
        /lapresse|bnnterminal|globeandmail/i.test(citation.url || citation)
      )
    );

    // Prendre 1-2 sources max pour ne pas alourdir
    const selectedSources = reliableSources.slice(0, 2);

    if (selectedSources.length > 0) {
      cleanedText += '\n\n📰 Pour approfondir:\n';

      selectedSources.forEach((citation, index) => {
        const url = typeof citation === 'string' ? citation : citation.url;
        const title = citation.title || extractDomainName(url);

        // Format amical: "📰 Bloomberg: [url]" ou "📰 Source 1: [url]"
        cleanedText += `📰 ${title}: ${url}\n`;
      });
    }
  }
  ...
  if (!isWatchlistRequest) {
    // Utiliser SEULEMENT les tickers explicitement fournis dans le contexte
    const contextTickers = context.tickers || [];

    // Filtrer pour garder seulement les vrais tickers (2-5 lettres, format valide)
    const validTickers = contextTickers.filter(t =>
      t &&
      typeof t === 'string' &&
      t.length >= 2 &&
      t.length <= 5 &&
      /^[A-Z]+(\.[A-Z]+)?$/.test(t) // Format: AAPL ou AC.TO
    );

    // Ajouter section graphiques si des tickers valides existent
    if (validTickers.length > 0) {
      const tickersList = validTickers.slice(0, 3); // Max 3 liens
      cleanedText += '\n\n📊 Graphiques:\n';
  ...
  // ✅ RETOUR COMPLET - Le découpage en plusieurs SMS est géré par sms.js/Twilio
  // Cela permet à Emma de fournir des analyses expertes complètes par SMS
  return cleanedText;
}
