...
export function adaptForSMS(text, context = {}) {
  ...
  // 8. LIENS TRADINGVIEW - DÉSACTIVÉ (sur demande utilisateur)
  // Les liens de graphiques automatiques ne sont plus ajoutés aux réponses
  // Si nécessaire à l'avenir, décommenter le bloc ci-dessous

  /* DÉSACTIVÉ - Ajout automatique de liens TradingView
  ...
  if (!isWatchlistRequest) {
    ...
    if (context?.tickers && Array.isArray(context.tickers) && context.tickers.length > 0) {
      tickersList = context.tickers.slice(0, 2);
      ...
      tickersList = validTickers.slice(0, 2);
...
export async function adaptForEmail(text, context = {}) {
  ...
  // ⚠️ OUTLOOK COMPATIBLE: Pas de inline-flex, utiliser vertical-align
  const sortedTickers = Array.from(logoMap.entries()).sort((a, b) => b[0].length - a[0].length);
  
  for (const [ticker, logoUrl] of sortedTickers) {
    // Format Outlook-compatible: image + texte alignés avec vertical-align
    // Pas de class, pas de onerror JS, pas de flex
    const logoHtml = `<span style="white-space: nowrap;"><img src="${logoUrl}" alt="${ticker}" width="24" height="24" style="display: inline; vertical-align: middle; border: 0; margin-right: 5px;" /><strong style="vertical-align: middle;">${ticker}</strong></span>`;

    // Remplacer les occurrences du ticker
    const escapedTicker = ticker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const tickerRegex = new RegExp(`\\b${escapedTicker}\\b(?![^<]*>|[^<]*</span>)`, 'g');
  ...
  // Headers avec styles inline Outlook-compatibles (pas de margin, utiliser padding)
  html = html.replace(/^# (.*$)/gim, `<tr><td style="color: #000000; font-size: 24px; font-weight: bold; font-family: ${fontStack}; padding: 24px 0 12px 0; border-left: 4px solid #1a73e8; padding-left: 12px;">$1</td></tr>`);
  html = html.replace(/^## (.*$)/gim, `<tr><td style="color: #000000; font-size: 20px; font-weight: bold; font-family: ${fontStack}; padding: 20px 0 10px 0;">$1</td></tr>`);
  html = html.replace(/^### (.*$)/gim, `<tr><td style="color: #1a1a1a; font-size: 16px; font-weight: bold; font-family: ${fontStack}; padding: 16px 0 8px 0;">$1</td></tr>`);

  // Bold et italique
  html = html.replace(/\*\*([^*]+)\*\*/g, `<strong style="font-family: ${fontStack}; font-weight: bold; color: #000000;">$1</strong>`);
  html = html.replace(/\*([^*]+)\*/g, `<em style="font-family: ${fontStack}; color: #333333;">$1</em>`);

  // Liens Outlook-compatibles
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, `<a href="$2" style="color: #0066cc; font-family: ${fontStack}; text-decoration: underline;">$1</a>`);

  // Bullet points
  html = html.replace(/^- (.*)$/gim, `<tr><td style="font-family: ${fontStack}; color: #000000; padding: 4px 0 4px 20px;">&bull; $1</td></tr>`);

  // Paragraphes - convertir en structure table
  html = html.replace(/\n\n/g, `</td></tr><tr><td style="font-family: ${fontStack}; color: #000000; line-height: 1.6; padding: 8px 0;">`);
  
  // Wrapper le contenu dans une table
  html = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td style="font-family: ${fontStack}; color: #000000; line-height: 1.6; padding: 8px 0;">${html}</td></tr>
  </table>`;

  // Template email complet - TABLE-BASED pour Outlook
  return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="utf-8">
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff;">
          ...
          <tr>
            <td style="background-color: #667eea; padding: 30px; text-align: center;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="font-size: 28px; font-weight: bold; color: #ffffff; font-family: Arial, Helvetica, sans-serif; text-align: center;">
                    Emma IA™
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="font-size: 14px; color: #ffffff; font-family: Arial, Helvetica, sans-serif; text-align: center; opacity: 0.95;">
                    Assistante Financière GOB
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 20px; text-align: center; border-top: 1px solid #e0e0e0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="font-size: 12px; color: #666666; font-family: Arial, Helvetica, sans-serif; text-align: center; padding-bottom: 4px;">
                    Analyse Financière Avancée
                  </td>
                </tr>
                <tr>
                  <td style="font-size: 12px; color: #888888; font-family: Arial, Helvetica, sans-serif; text-align: center;">
                    Propulsé par Emma IA™ &amp; JSLAI™ Score
                  </td>
                </tr>
              </table>
            </td>
