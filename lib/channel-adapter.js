...
export async function adaptForEmail(text, context = {}) {
  ...
  // Headers avec polices Bloomberg-style
  const bloombergFontStack = "'Helvetica Neue', Helvetica, Arial, 'Inter', sans-serif";
  html = html.replace(/^### (.*$)/gim, `<h3 style="color: #1a73e8; margin-top: 20px; font-family: ${bloombergFontStack}; font-weight: 600; font-size: 16px;">$1</h3>`);
  html = html.replace(/^## (.*$)/gim, `<h2 style="color: #1a73e8; margin-top: 24px; font-family: ${bloombergFontStack}; font-weight: 700; font-size: 20px;">$1</h2>`);
  html = html.replace(/^# (.*$)/gim, `<h1 style="color: #1a73e8; margin-top: 28px; font-family: ${bloombergFontStack}; font-weight: 700; font-size: 24px; letter-spacing: -0.3px;">$1</h1>`);

  // Bold et italique avec polices Bloomberg
  html = html.replace(/\*\*([^*]+)\*\*/g, `<strong style="font-family: ${bloombergFontStack}; font-weight: 600;">$1</strong>`);
  html = html.replace(/\*([^*]+)\*/g, `<em style="font-family: ${bloombergFontStack};">$1</em>`);

  // Liens avec style Bloomberg
  html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, `<a href="$2" style="color: #1a73e8; font-family: ${bloombergFontStack}; text-decoration: none; border-bottom: 1px solid #1a73e8;">$1</a>`);

  // Bullet points avec polices Bloomberg
  html = html.replace(/^- (.*)$/gim, `<li style="font-family: ${bloombergFontStack};">$1</li>`);
  html = html.replace(/(<li.*<\/li>)/s, `<ul style="margin-left: 20px; font-family: ${bloombergFontStack};">$1</ul>`);

  // Paragraphes avec polices Bloomberg
  html = html.replace(/\n\n/g, `</p><p style="font-family: ${bloombergFontStack};">`);
  html = `<p style="font-family: ${bloombergFontStack};">` + html + '</p>';
...
<body style="font-family: 'Helvetica Neue', Helvetica, Arial, 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
    ...
    <p style="color: rgba(255,255,255,0.9); margin: 5px 0 0 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-weight: 400;">Assistante Financière GOB</p>
  </div>
  ...
  <div style="text-align: center; margin-top: 20px; padding: 20px; color: #666; font-size: 12px; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
    <p style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">Groupe Ouellet Bolduc - Analyse Financière Avancée</p>
    <p style="margin: 5px 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">Propulsé par Emma IA™ & JSLAI™ Score</p>
  </div>
</body>
</html>
  `.trim();
