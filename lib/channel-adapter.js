...
export function adaptForSMS(text) {
  ...
  cleanedText = cleanedText.replace(/https?:\/\/finviz\.com\/chart\.ashx[^\s)]+/g, '[Graphique Finviz]');

  // 3. Supprimer les URLs d'images génériques
  cleanedText = cleanedText.replace(/https?:\/\/[^\s)]+\.(png|jpg|jpeg|gif|svg|webp)[^\s)]*/gi, '[Image]');
  ...
  // 7. Tronquer intelligemment au dernier point avant la limite
  const truncated = cleanedText.substring(0, MAX_LENGTH);
  ...
  if (cutPoint > MAX_LENGTH * 0.7) {
    ...
    return cleanedText.substring(0, cutPoint + 1) + '\n\n...(Tronqué pour SMS)';
  }

  // Sinon, coupe brutale avec ellipse
  return truncated.substring(0, MAX_LENGTH - 25) + '...\n\n(Tronqué pour SMS)';
}
