...
export function adaptForSMS(text, context = {}) {
  ...
  // 7. AJOUTER LIENS TRADINGVIEW pour graphiques interactifs
  // Extraire les tickers mentionnés dans le texte
  const tickerRegex = /\b([A-Z]{1,5})\b/g;
  const tickersInText = new Set();
  let match;
  while ((match = tickerRegex.exec(cleanedText)) !== null) {
    const ticker = match[1];
    // Filtrer les faux positifs (mots communs en majuscules)
    if (ticker.length >= 2 && !['CEO', 'CFO', 'USA', 'USD', 'EUR', 'CAD', 'GDP', 'FED', 'SEC', 'IPO', 'ETF', 'API', 'AI', 'IT'].includes(ticker)) {
      tickersInText.add(ticker);
    }
  }

  // Ajouter tickers du contexte (watchlist, team, etc.)
  const contextTickers = [
    ...(context.tickers || []),
    ...(context.user_watchlist || []),
    ...(context.team_tickers || [])
  ];
  contextTickers.forEach(t => tickersInText.add(t));

  // Ajouter section graphiques si des tickers sont détectés
  if (tickersInText.size > 0) {
    const tickersList = Array.from(tickersInText).slice(0, 3); // Max 3 liens pour éviter spam
    cleanedText += '\n\n📊 Graphiques:\n';
