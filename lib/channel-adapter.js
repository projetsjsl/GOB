...
export function adaptForSMS(text, context = {}) {
  ...
  // 🚨 OPTIMISATION SMS ÉCONOMIQUE: Remplacer emojis complexes par ASCII pour GSM-7
  // Cela réduit le coût des SMS de 2.3x (UCS-2 70 chars → GSM-7 160 chars)
  ...
  // Emojis communs → ASCII équivalent (garder lisibilité)
  ...
  // Utiliser MAJUSCULES pour les titres de sections importants
  cleanedText = cleanedText.replace(/^(\d+)\.\s+([^\n]+)/gm, (match, num, title) => {
    // Garder les titres courts en majuscules pour emphase
    if (title.length < 40) {
  ...
  // Mettre en évidence les ratios financiers (P/E, ROE, etc.)
  ...
  // Ajouter séparateurs visuels entre sections (compatible GSM-7)
  cleanedText = cleanedText.replace(/\n\n(\d+\.)/g, '\n\n---\n$1');
  
  // Mettre en évidence les chiffres importants ($, %, x)
  cleanedText = cleanedText.replace(/(\$[\d,]+\.?\d*)/g, '[$1]'); // $68.30 → [$68.30]
  cleanedText = cleanedText.replace(/([+-]?\d+\.?\d*%)/g, '[$1]'); // -11% → [-11%]
