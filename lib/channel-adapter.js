...
export async function adaptForEmail(text, context = {}) {
  ...
  for (const [ticker, logoUrl] of sortedTickers) {
    ...
    //           <strong>AAPL</strong>
    //         </span>
    const logoHtml = `<span style="display: inline-flex; align-items: center; gap: 6px; margin: 0 2px;">
    ...
    // Utiliser un placeholder temporaire pour éviter remplacements multiples
    const tickerRegex = new RegExp(`\\b${ticker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b(?![^<]*>|[^<]*</|data-ticker-replaced)`, 'g');
    html = html.replace(tickerRegex, (match) => {
      // Vérifier si on est déjà dans un span de logo
      const beforeMatch = html.substring(0, html.indexOf(match));
      const lastSpan = beforeMatch.lastIndexOf('<span style="display: inline-flex');
      const lastSpanClose = beforeMatch.lastIndexOf('</span>');
      if (lastSpan > lastSpanClose) {
        // On est dans un span existant, ne pas remplacer
        return match;
      }
      return logoHtml.replace('>', ' data-ticker-replaced="true">');
    });
  }
  
  // Nettoyer les placeholders
  html = html.replace(/ data-ticker-replaced="true"/g, '');
