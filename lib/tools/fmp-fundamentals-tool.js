...
export default class FMPFundamentalsTool extends BaseTool {
    ...
    /**
     * Detect product type based on RELIABLE company data only
     * Returns: { type, category, framework }
     *
     * IMPORTANT: This only uses FMP's explicit flags and industry data.
     * For more accurate detection (ETF, Mutual Funds, etc.), use Yahoo Quote Type Tool.
     */
    _detectProductType(company, ticker) {
        const nameUpper = (company.companyName || '').toUpperCase();
        const tickerUpper = ticker.toUpperCase();

        // 1. ETF detection (FMP has explicit isEtf flag - RELIABLE)
        ...
        // 2. REIT detection (FMP has explicit industry field - RELIABLE)
        if (company.industry === 'REIT' || company.industry === 'Real Estate' ||
            nameUpper.includes('REIT') || nameUpper.includes('REAL ESTATE INVESTMENT TRUST')) {
            return {
                type: 'REIT',
        ...
        // 3. Mutual Fund detection - ONLY from explicit name indicators
        // (For accurate mutual fund detection, use Yahoo Quote Type Tool)
        if (nameUpper.includes('MUTUAL FUND') || nameUpper.includes('FUND TRUST')) {
            return {
                type: 'Mutual Fund (tentative)',
                category: 'Fund',
                framework: 'fund_analysis',
                needs_verification: true  // Flag that Yahoo Quote Type should be used
            };
        }

        // 4. Bond/Fixed Income - Only from explicit name
        if (nameUpper.includes('TREASURY') || nameUpper.includes('U.S. BOND') ||
            (nameUpper.includes('BOND') && !nameUpper.includes('BONDING'))) {
            return {
                type: 'Bond/Fixed Income (tentative)',
        ...
        // 5. Preferred Stock - Only from explicit ticker pattern (-P, -PR)
        if (tickerUpper.match(/[-.]PR?$/) || nameUpper.includes('PREFERRED STOCK')) {
            return {
        ...
        // 6. ADR - Only from explicit name
        if (nameUpper.includes('AMERICAN DEPOSITARY') || nameUpper.includes('ADR')) {
            return {
                type: 'ADR',
                category: 'Equity',
        ...
        // Default: Common Stock (but recommend using Yahoo Quote Type for verification)
        return {
            type: 'Common Stock',
            category: 'Equity',
            framework: 'stock_analysis',
            needs_verification: false  // Common stock is default, usually safe
        };
    }
}
