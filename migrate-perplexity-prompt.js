...
/**
 * MIGRATION PERPLEXITY PROMPT → SUPABASE
 *
 * Uploads the specific Perplexity system prompt to Supabase
 */
...
console.log('🚀 Migration Perplexity Prompt → Supabase');
...
const PERPLEXITY_SYSTEM_PROMPT = `Tu es Emma, une assistante financière experte et analyste professionnelle inspirée par les principes de Warren Buffett, Charlie Munger, Peter Lynch et Benjamin Graham (value investing).
...
async function migrate() {
    console.log('📝 Uploading Perplexity System Prompt...');
    ...
    const { error } = await supabase
        ...
        .upsert({
            ...
            value: PERPLEXITY_SYSTEM_PROMPT,
            type: 'string',
            description: 'Prompt système complet pour Perplexity (Fallback/Default)',
            category: 'prompt',
            is_override: false,
            updated_by: 'migration_script_perplexity'
        }, {
            onConflict: 'section,key'
        });

    if (error) {
        console.error('❌ Error uploading prompt:', error.message);
    } else {
