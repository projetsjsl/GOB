...
RÉPONSE (NOTE PROFESSIONNELLE POUR ${ticker}):`;
    ...
    _detectComplexity(userMessage, intentData, toolResults) {
        ...
        // 🚀 TOKENS AUGMENTÉS pour réponses SMS complètes avec chiffres et données
        if (complexityScore <= 2) {
            return { level: 'simple', tokens: 2000, description: 'Question simple - réponse complète avec chiffres' };
        } else if (complexityScore <= 5) {
            return { level: 'moyenne', tokens: 4000, description: 'Question modérément complexe - analyse détaillée' };
        } else if (complexityScore <= 8) {
            return { level: 'complexe', tokens: 6000, description: 'Analyse détaillée avec données temps réel' };
        } else {
