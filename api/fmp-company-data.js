/**
 * API Proxy pour recuperer les donnees completes d'une compagnie
 * Utilise FMP Premium pour: profile, key-metrics, prix historiques
 * 
 * Premium Features:
 * - Historique etendu: 20 ans (7300 jours) au lieu de 5 ans
 * - Key Metrics: 30 annees au lieu de 20
 * - Annual Data: 15 dernieres annees au lieu de 6
 * 
...
export default async function handler(req, res) {
    ...
    // Mapping de symboles alternatifs pour les cas speciaux (fallback si Search echoue)
    const symbolVariants = {
        ...
        'MRU': ['MRU.TO', 'MRU'],
        'ABX': ['ABX.TO', 'ABX', 'GOLD'], // Barrick Gold peut etre GOLD maintenant
        'IFC': ['IFC.TO', 'IFC'],
        'GWO': ['GWO.TO', 'GWO'],
        'EMA': ['EMA.TO', 'EMA'],
        'CCA': ['CCA.TO', 'CCA'],
    ...
    // Fonction pour utiliser FMP Search Premium pour resoudre automatiquement les symboles
        ...
        try {
            ...
            if (searchRes.ok) {
                ...
                if (Array.isArray(searchData) && searchData.length > 0) {
                    // Trouver le meilleur match (exact ou le premier resultat)
            ...
            console.warn(` FMP Search failed for ${symbolToSearch}, using fallback:`, error.message);
        }
        return null;
    };
    ...
    const tryFetchProfile = async (symbolToTry, retryCount = 0) => {
        const MAX_RETRIES = 3;
        const RETRY_DELAY = 5000; // 5 secondes (augmente pour eviter les cascades de 429)
        ...
        try {
            ...
            if (!profileRes.ok) {
                ...
                console.warn(` FMP Profile HTTP error for ${symbolToTry}: ${profileRes.status} - ${errorText.substring(0, 200)}`);
                ...
                if (profileRes.status === 401 || profileRes.status === 403) {
                    try {
                        ...
                        if (errorJson['Error Message'] && errorJson['Error Message'].includes('Invalid API KEY')) {
                            console.error(` FMP API KEY INVALIDE - Verifiez FMP_API_KEY dans Vercel`);
                        }
                    } catch (e) {
                        // Ignore parse error
                    }
                ...
                // Si c'est un rate limit apres tous les retries, retourner une erreur speciale
                if (profileRes.status === 429) {
                    throw new Error(`Rate limit FMP atteint pour ${symbolToTry} apres ${MAX_RETRIES} tentatives`);
                }
            ...
            // Verifier si c'est un objet d'erreur FMP
            if (profileData && typeof profileData === 'object' && !Array.isArray(profileData)) {
                if (profileData['Error Message']) {
                    console.warn(` FMP Error for ${symbolToTry}: ${profileData['Error Message']}`);
                    return null;
                }
            }
            
            // Verifier si c'est un tableau vide ou invalide
            if (!profileData || !Array.isArray(profileData) || profileData.length === 0) {
                console.warn(` FMP Profile returned empty/invalid data for ${symbolToTry}`);
                return null;
            }
            
            // Verifier que le profil a des donnees valides
            const profile = profileData[0];
            if (!profile || !profile.symbol) {
                console.warn(` FMP Profile data invalid for ${symbolToTry}`);
                return null;
            }
    ...
    try {
        ...
        // 2. Si echec et qu'on a des variantes, les essayer
        ...
        // 3. Si toujours echec, essayer avec .TO pour les symboles canadiens
        ...
        // 4. Si toujours echec, essayer sans le suffixe de classe
        if (!profileResult && (cleanSymbol.includes('.B') || cleanSymbol.includes('.A'))) {
        ...
        // 5. Pour les symboles sans suffixe, essayer .TO si pas deja fait
        ...
        if (!profileResult) {
            ...
            // Verifier si c'est un probleme de cle API en testant avec un ticker connu
            ...
            try {
                ...
                if (!testRes.ok) {
                    ...
                    if (testError.includes('Invalid API KEY') || testError.includes('API key')) {
                        apiKeyValid = false;
                        console.error(` FMP API KEY semble invalide - Verifiez FMP_API_KEY dans Vercel`);
                        return res.status(500).json({
                            error: 'FMP API key invalid or not configured',
                            message: 'La cle API FMP semble invalide. Verifiez FMP_API_KEY dans les variables d\'environnement Vercel.',
                            tried: triedSymbols,
                            diagnostic: 'Test avec AAPL a egalement echoue'
                        });
                    }
                    // Si AAPL echoue mais ce n'est pas une erreur de cle, c'est peut-etre un rate limit
                    if (testRes.status === 429) {
                        console.error(` Rate limiting detecte lors du test avec AAPL - FMP peut etre surcharge`);
                        return res.status(429).json({
                            error: 'Rate limit FMP',
                            message: `Rate limiting detecte. Veuillez reessayer dans quelques instants.`,
                            tried: triedSymbols,
                            diagnostic: 'Test avec AAPL a egalement echoue avec 429'
                        });
                    }
                } else {
                    const testData = await testRes.json();
                    if (testData && Array.isArray(testData) && testData.length > 0) {
                        console.log(` Test API reussi avec AAPL - La cle API fonctionne`);
                    }
                }
            } catch (testError) {
            ...
            return res.status(404).json({ 
                ...
                tried: triedSymbols,
                message: `Aucune donnee trouvee pour ${cleanSymbol} apres avoir essaye ${triedSymbols.length} variante(s). Verifiez que le symbole est correct et disponible dans FMP.`,
        ...
        results.forEach((r, i) => {
            if (r.status === 'rejected') {
                console.warn(` Promise ${i} rejected for ${usedSymbol}:`, r.reason);
            }
        });
            ...
            if (metricsRes.ok) {
                ...
                if (metricsJson && typeof metricsJson === 'object' && !Array.isArray(metricsJson)) {
                    if (metricsJson['Error Message']) {
                        console.warn(` FMP Key Metrics Error (limit=30): ${metricsJson['Error Message']}`);
                    }
                } else if (Array.isArray(metricsJson)) {
                    metricsData = metricsJson;
                }
            } else {
                console.warn(` FMP Key Metrics HTTP ${metricsRes.status} for ${usedSymbol} (limit=30)`);
            }
        } catch (metricsError) {
            console.warn(` FMP Key Metrics parse error for ${usedSymbol}: ${metricsError.message}`);
        }
        ...
        if (metricsData.length === 0) {
            console.log(` Retrying Key Metrics for ${usedSymbol} with limit=15 (intermediate fallback)...`);
            try {
        ...
        if (metricsData.length === 0) {
            console.log(` Retrying Key Metrics for ${usedSymbol} with limit=5 (final fallback)...`);
            try {
        ...
        if (metricsData.length === 0) {
            console.warn(` No Key Metrics data for ${usedSymbol} - profile data will still be returned`);
        }
        ...
        if (dividendRes.ok) {
            ...
            console.warn(` No dividend data available for ${cleanSymbol}`);
        }
        ...
        if (!priceRes.ok) {
            ...
            try {
               ...
               console.warn(` FMP Historical Price warning for ${usedSymbol}: ${priceRes.status} - ${errorText.substring(0, 200)} (continuing without price history)`);
            } catch (e) {
               console.warn(` FMP Historical Price warning for ${usedSymbol}: ${priceRes.status} (continuing without price history)`);
            }
        }
        ...
        try {
            if (priceRes.ok) {
                ...
                if (json && typeof json === 'object' && !Array.isArray(json) && json['Error Message']) {
                     console.warn(` FMP Historical Price API returned error: ${json['Error Message']}`);
                } else if (json) {
                    priceData = json;
                }
            }
        } catch (priceParseError) {
            console.warn(` FMP Historical Price JSON parse error: ${priceParseError.message}`);
        }
        ...
        if ((!currentPrice || currentPrice === 0) && priceData && priceData.historical && priceData.historical.length > 0) {
            ...
            console.log(` Used latest historical closing price (${currentPrice}) as current price fallback`);
        }
        ...
        if (metricsData && metricsData.length > 0) {
            const mostRecentMetric = metricsData[0]; // Le plus recent (premier apres tri)
            
            // Beta est generalement dans les key-metrics les plus recents
            ...
            // ROE (Return on Equity) - generalement en pourcentage dans FMP
            roe = mostRecentMetric?.roe || mostRecentMetric?.returnOnEquity || null;
            if (roe !== null && roe !== undefined) {
                roe = parseFloat(roe);
                // Si ROE est deja en pourcentage (0-100), le garder tel quel
                // Si ROE est en decimal (0-1), le convertir en pourcentage
                if (Math.abs(roe) <= 1 && roe !== 0) {
                    roe = roe * 100;
                }
            }
            
            // ROA (Return on Assets) - generalement en pourcentage dans FMP
            roa = mostRecentMetric?.roa || mostRecentMetric?.returnOnAssets || null;
            if (roa !== null && roa !== undefined) {
                roa = parseFloat(roa);
                // Si ROA est deja en pourcentage (0-100), le garder tel quel
                // Si ROA est en decimal (0-1), le convertir en pourcentage
                if (Math.abs(roa) <= 1 && roa !== 0) {
                    roa = roa * 100;
                }
            }
            
            //  NOUVEAU: Recuperer le dividende actuel depuis key metrics
            ...
            // 2. Fallback: Calculer a partir de dividendYield et currentPrice
                ...
                if (yieldPercent > 0 && yieldPercent < 50) { // Raisonnable: 0-50%
                    ...
                    console.log(` Calcule currentDividend depuis dividendYield (${yieldPercent.toFixed(2)}%): ${currentDividendFromMetrics.toFixed(4)}`);
                }
            }
        }
        ...
        const getPreferredSymbol = (profile) => {
            ...
            if (country === 'CA' || exchange === 'TSX' || exchange === 'TSXV') {
                ...
                // Pour l'instant, on garde le symbole actuel mais on pourrait chercher l'equivalent TSX
                return symbol;
            }
            
            // Pour les titres europeens ou asiatiques, prioriser l'ADR sur bourse US
            ...
            if (europeanCountries.includes(country) || asianCountries.includes(country)) {
                // Si le symbole actuel est deja sur une bourse US (NASDAQ, NYSE, etc.), on le garde
        ...
        // Generer plusieurs URLs de logo possibles pour le fallback
        ...
        const mappedInfo = {
            ...
            securityRank: 'N/A', // A mettre a jour manuellement ou via ValueLine
            marketCap: profile.mktCap ? formatMarketCap(profile.mktCap) : 'N/A',
            logo: logoUrl,
            country: profile.country || '',
            exchange: profile.exchangeShortName || profile.exchange || '',
            currency: profile.currency || 'USD',
            preferredSymbol: cleanSymbol, // Garder le symbole original demande
            actualSymbol: usedSymbol, // Le symbole reellement utilise par FMP
            logoSymbol: logoBaseSymbol, // Symbole a utiliser pour les logos (sans .TO, avec format standard)
            beta: beta, // Ajouter le beta recupere
            roe: roe, // Return on Equity (en pourcentage)
            roa: roa // Return on Assets (en pourcentage)
        };

        //  NOUVEAU: Calculer le dividende actuel pour l'inclure dans la reponse
        // 1. Utiliser le dividende depuis key metrics si disponible
        let finalCurrentDividend = currentDividendFromMetrics;
        
        // 2. Fallback: Utiliser le dividende de l'annee la plus recente avec dividende > 0
        ...
        // 3. Fallback final: Calculer a partir du yield moyen historique si disponible
        if (!finalCurrentDividend || finalCurrentDividend === 0) {
            ...
            if (yearsWithDividend.length > 0 && currentPrice > 0) {
                ...
                if (avgYield > 0.1 && avgYield < 20) {
                    ...
                    console.log(`i ${usedSymbol}: Dividende estime a partir du yield moyen historique (${avgYield.toFixed(2)}%): ${finalCurrentDividend.toFixed(4)}`);
                }
            }
        }
        ...
        return res.status(200).json({
            ...
            currentDividend: parseFloat(finalCurrentDividend.toFixed(4)) //  NOUVEAU: Dividende actuel calcule
        });
