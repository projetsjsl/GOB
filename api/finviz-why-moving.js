 ...
 * Analyse news, réseaux sociaux, SEC filings, et discussions de forums
 */
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Cette section se trouve généralement dans la section News, avec un format spécifique
        ...
        const whyMovingPatterns = [
            ...
            // Pattern générique pour explication de mouvement
        ...
        // Cette section se trouve généralement en haut de la page de cotation
        ...
        if (newsTableMatch) {
            ...
            for (const pattern of dateExplanationPatterns) {
                ...
                if (match) {
                    // Le premier match est généralement la date, le second l'explication
                    if (match[1] && match[2]) {
            ...
            // Si pas trouvé avec date, chercher juste la première explication significative
            if (!explanation) {
        ...
        // Pattern 3: Si toujours pas trouvé, chercher dans toute la page HTML
        if (!explanation) {
        ...
        // Si pas trouvé dans la section News, chercher dans toute la page
        if (!explanation) {
            // Chercher des patterns d'explication génériques
        ...
        if (explanation && process.env.GEMINI_API_KEY) {
            try {
                ...
                console.warn('⚠️ AI enrichment failed, using original explanation');
            }
        }
        ...
        if (!explanation) {
            console.log(`⚠️ No "Why Is It Moving?" explanation found for ${ticker}`);
...
async function enrichWithAI(ticker, explanation) {
    try {
        ...
        const response = await fetch(`${baseUrl}/api/gemini/chat`, {
            ...
            body: JSON.stringify({
                message: `Explique brièvement (2-3 phrases) pourquoi ${ticker} bouge selon cette information: "${explanation}". Sois concis et factuel.`,
