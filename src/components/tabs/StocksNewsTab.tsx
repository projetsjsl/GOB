...
const StocksNewsTab: React.FC<TabProps> = (props) => {
        ...
        const LucideIcon = (props as any).LucideIcon || 
                          (typeof window !== 'undefined' ? (window as any).LucideIcon : undefined) || 
                          (typeof window !== 'undefined' ? (window as any).DASHBOARD_UTILS?.LucideIcon : undefined) || 
                          (({ name, className = '' }: any) => <span className={`icon-${name} ${className}`}></span>);
            ...
            {tickers.length > 0 && (
                <div className={`mt-6 p-6 rounded-xl transition-colors duration-300 ${
                    ...
                    <h3 className={`text-xl font-bold mb-4 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        <LucideIcon name="Fire" className="w-6 h-6" style={{ color: 'var(--theme-text)' }} />
                        Top Movers du Jour
                    </h3>
