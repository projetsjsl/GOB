...
const GroupChatTab: React.FC<TabProps> = () => {
    ...
    const [accessSafety, setAccessSafety] = useState<'token' | 'needs-token'>('needs-token');
    const [sessionOrigin, setSessionOrigin] = useState<string>('non configuré');
    const hasEnvChatUrl = Boolean(DEFAULT_CHAT_URL);
    const isUsingEnvDefault = Boolean(DEFAULT_CHAT_URL) && settings.sessionUrl === DEFAULT_CHAT_URL;
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    {settings.sessionUrl ? (
                                        ...
                                        <span className="px-2 py-1 rounded-md bg-gray-900/70 text-gray-200 text-xs border border-gray-700">Lien à renseigner</span>
                                    )}
                                </div>
                            ...
                            {settings.sessionUrl ? (
                                ...
                                <div className="absolute inset-0 flex items-center justify-center text-center p-6">
                                    <div className="space-y-2 max-w-xl">
                                        <p className="font-semibold text-white">Configurez le lien du salon</p>
                                        <p className="text-gray-300 text-sm">
                                            Ajoutez l’URL de clavardage partagée dans le formulaire ou via la variable d’environnement{' '}
                                            <code className="px-1 py-0.5 rounded bg-gray-900 border border-gray-700">VITE_GROUP_CHAT_URL</code> pour activer la prévisualisation.
                                        </p>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label className="space-y-1">
                                ...
                                <p className="text-xs text-blue-200">
                                    Source par défaut : variable d’environnement <code className="px-1 py-0.5 rounded bg-gray-900 border border-gray-700">VITE_GROUP_CHAT_URL</code> (<code className="px-1 py-0.5 rounded bg-gray-900 border border-gray-700">.env</code> ou Vercel). {hasEnvChatUrl
                                        ? isUsingEnvDefault
