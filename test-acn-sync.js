...
const autoFillAssumptionsFromFMPData = (data, currentPrice, existingAssumptions = {}) => {
  // ✅ FIX: Guard clause for empty or invalid data (matches public/3p1/utils/calculations.ts)
  ...
  if (!lastValidData) {
      console.warn('⚠️ No valid data found for assumptions');
      return existingAssumptions || {};
  }
...
async function syncTicker(tickerObj) {
  ...
  try {
    ...
    // ✅ CRITICAL POINT: calling the function that was crashing
...
async function run() {
    console.log("🚀 Starting Targeted Test for ACN...");
